...
function get_jobs(type){
  ...
  switch(type){
    case "search":
      ...
      var uri="https://job-portal-app-demo.herokuapp.com/search?q="+input;
      break;
    case "all":
      var uri="https://job-portal-app-demo.herokuapp.com/list_all";
  }
...
function login(){
  var xhr = new XMLHttpRequest();
  var uri="https://job-portal-app-demo.herokuapp.com/login";
...
function post_job(){
  var xhr = new XMLHttpRequest();
  var uri="https://job-portal-app-demo.herokuapp.com/add";
  xhr.open("POST", uri, true);
  xhr.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
  xhr.onreadystatechange = function (e) {
    if (this.readyState === XMLHttpRequest.DONE && this.status === 200) {
